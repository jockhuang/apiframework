...
public class DefaultHttpRequest extends DefaultHttpMessage implements MutableHttpRequest {
    ...
    public void setParameters(String queryString) {
        try {
            ...
            throw new InternalError(HttpCodecUtils.DEFAULT_CHARSET_NAME + " decoder must be provided by JDK.");
        }
    }
    ...
    public void setParameters(String queryString, String encoding) throws UnsupportedEncodingException {
        ...
        while (pos < queryString.length()) {
            ...
            if (equalPos < 0) {
                ...
                this.addParameter(URLDecoder.decode(value.substring(0, equalPos), encoding),
    ...
    public void setContent(IoBuffer content) {
        ...
        if (HttpHeaderConstants.VALUE_URLENCODED_FORM.equalsIgnoreCase(getContentType())) {
            content.mark();
            try {
    ...
    private static final ThreadLocal<DateFormat> EXPIRY_FORMAT_LOACAL = new ThreadLocal<DateFormat>() {
        ...
        protected DateFormat initialValue() {
            ...
            format.setTimeZone(TimeZone.getTimeZone(HttpCodecUtils.DEFAULT_TIME_ZONE_NAME));
            return format;
        }
    ...
    public void normalize() {
            ...
            try {
                ...
                for (Map.Entry<String, List<String>> e : params.entrySet()) {
                    ...
                    for (String v : e.getValue()) {
                        ...
                        buf.append(URLEncoder.encode(e.getKey(), HttpCodecUtils.DEFAULT_CHARSET_NAME));
                        buf.append('=');
                        buf.append(URLEncoder.encode(v, HttpCodecUtils.DEFAULT_CHARSET_NAME));
                        first = false;
                    }
                }
                ...
                if (buf.length() > 0) {
                    ...
                    default:
                        if (queryIndex >= 0) {
                            setRequestUri(new URI(uri.substring(0, queryIndex + 1) + buf));
                        } else {
        ...
        if (!cookies.isEmpty()) {
            ...
            for (Cookie c : cookies) {
                ...
                if (expiry >= 0) {
                    if (version == 0) {
                        String expires = expiry == 0 ? EXPIRED_DATE : getFormattedExpiry(System.currentTimeMillis()
                                + 1000 * expiry);
                        buf.append("; Expires=");
                        buf.append(expires);
                    } else {
        ...
        if (!containsHeader(HttpHeaderConstants.KEY_HOST)) {
            ...
            if (host != null) {
                if ((uri.getScheme().equalsIgnoreCase("http") && uri.getPort() != 80 && uri.getPort() > 0)
                        || (uri.getScheme().equalsIgnoreCase("https") && uri.getPort() != 443 && uri.getPort() > 0)) {
