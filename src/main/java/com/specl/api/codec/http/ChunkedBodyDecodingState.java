...
/**
 * A decoder which decodes the body of HTTP Requests having a "chunked" transfer-coding.
 * 
 * This decoder does <i>not</i> decode trailing entity-headers - it simply discards them. Tomcat currently does the same
 * - so this is probably the most stable approach for now.<br/>
 * If the need arises to decode them in the future, we simply need to employ a <code>HttpHeaderDecoder</code> following
 * the last chunk - yielding headers for the encountered trailing entity-headers.
 * <p/>
 * 
 ...
 * </pre>
 * 
 * <code>ChunkedBodyDecoder</code> employs a <code>SharedBytesAllocator</code> to enable the content of each decoded
 * chunk to contribute to a single <code>Bytes</code>. This enables all chunks to be read without requiring copying.
 * 
 * @author The Apache MINA Project (dev@mina.apache.org)
 * @version $Rev: 612015 $, $Date: 2008-01-15 18:06:51 +1300 (Tue, 15 Jan 2008) $
 */
abstract class ChunkedBodyDecodingState extends DecodingStateMachine {
    ...
    private final DecodingState READ_CHUNK_LENGTH = new ConsumeToDynamicTerminatorDecodingState() {
        ...
        protected DecodingState finishDecode(IoBuffer product, ProtocolDecoderOutput out) throws Exception {
            ...
            return AFTER_SKIP_CHUNK_EXTENSION.decode(IoBuffer.wrap(new byte[] { '\r' }), out);
        }
    ...
    private final DecodingState AFTER_SKIP_CHUNK_EXTENSION = new CrLfDecodingState() {
        ...
        protected DecodingState finishDecode(boolean foundCRLF, ProtocolDecoderOutput out) throws Exception {
            if (!foundCRLF) {
                HttpCodecUtils.throwDecoderException("Expected CRLF at the end of chunk extension.",
    ...
    private final DecodingState AFTER_CHUNK_DATA = new CrLfDecodingState() {
        ...
        protected DecodingState finishDecode(boolean foundCRLF, ProtocolDecoderOutput out) throws Exception {
            if (!foundCRLF) {
                HttpCodecUtils.throwDecoderException("Expected CRLF after a chunk data.",
                        HttpResponseStatus.BAD_REQUEST);
