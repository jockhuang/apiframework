 ...
 * @version $Rev: 612014 $, $Date: 2008-01-15 17:41:31 +1300 (Tue, 15 Jan 2008) $
 */
abstract class HttpRequestDecodingState extends DecodingStateMachine {
    ...
    private final DecodingState READ_HEADERS = new HttpHeaderDecodingState() {
        ...
        private int getContentLength(HttpRequest request) throws ProtocolDecoderException {
            int length = 0;
            String lengthValue = request.getHeader(HttpHeaderConstants.KEY_CONTENT_LENGTH);
            if (lengthValue != null) {
                try {
        ...
        private boolean isChunked(HttpRequest request) throws ProtocolDecoderException {
            boolean isChunked = false;
            String coding = request.getHeader(HttpHeaderConstants.KEY_TRANSFER_ENCODING);
            if (coding == null) {
                coding = request.getHeader(HttpHeaderConstants.KEY_TRANSFER_CODING);
            }
